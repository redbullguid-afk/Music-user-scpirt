...
local MonsterAdvice = {
    ...
    ["Giggle"] = "Né trần nhà ra hoặc ném Glowstick!", ["Gloombat"] = "Tắt nguồn sáng ngay!", ["Grumble"] = "Giữ khoảng cách, trốn sau vách núi!"
}
                        ...
                        if parent then
                            ...
                            if targetPart then
                                ...
                                if dist <= MAX_LOOT_DIST then
                                    ...
                                    if isLootItem then
                                        safeInteract(prompt)
                                    elseif (nameLower:find("drawer") or pNameLower:find("drawer") or nameLower:find("knob") or nameLower:find("closet") or nameLower:find("chest")) then
...
local function checkHiddenDrawerLoot(containerObj)
    pcall(function()
        ...
        local isContainer = nameLower:find("drawer") or nameLower:find("cabinet") or nameLower:find("counter") 
            or nameLower:find("desk") or nameLower:find("chest") or nameLower:find("locker")

        if not isContainer then return end

        local targetPart = containerObj:IsA("BasePart") and containerObj or containerObj:FindFirstChildWhichIsA("BasePart")
        ...
        for _, prompt in ipairs(containerObj:GetDescendants()) do
            if prompt:IsA("ProximityPrompt") then
                ...
                for realName, displayName in pairs(ImportantItems) do
                    if objText:find(realName) or actText:find(realName) then
                        foundItemName = displayName
                        break
                    end
                end

                if not foundItemName then
                    if objText:lower():find("gold") or objText:lower():find("coin") or actText:lower():find("gold") then
                        foundItemName = "💰 Tiền Trong Tủ"
                    end
                end
            end
        end
        ...
        if not foundItemName then
            ...
            if attrVal and ImportantItems[tostring(attrVal)] then
                foundItemName = ImportantItems[tostring(attrVal)]
            end
        end
        ...
        if not foundItemName then
            for _, child in ipairs(containerObj:GetChildren()) do
                if ImportantItems[child.Name] then
                    foundItemName = ImportantItems[child.Name]
                    break
                end
            end
        end

        if foundItemName and not containerObj:FindFirstChild("Mote_ESP_ESPItems", true) then
            createBillboard(targetPart, "📥 " .. foundItemName, ESPColors.Items, "ESPItems")
        end
    end)
end
...
local function processObject(obj)
    pcall(function()
        ...
        elseif nameLower:find("giggle") then detectedMonsterName = "Giggle"
        elseif nameLower:find("gloombat") then detectedMonsterName = "Gloombat"
        elseif nameLower:find("grumble") then detectedMonsterName = "Grumble"
        end
...
createToggleSwitch(pages[2], "🔵 ESP Vật Phẩm (Floor 1, 2 & Hộc Tủ)", "ESPItems", 40)
createToggleSwitch(pages[2], "🔴 ESP Quái Vật (Floor 1 & Floor 2)", "ESPMonster", 75)
