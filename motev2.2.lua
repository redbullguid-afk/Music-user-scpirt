-- ==================================================
-- MOTEHUB V6.0 - DOORS ULTIMATE ALL-IN-ONE
-- ==================================================
...
-- Bảng trạng thái Bật/Tắt các tính năng
local Flags = {
    AntiAFK = true,
    FlyCarpet = false,
    Fullbright = false,
...
--------------------------------------------------
-- 1. TÍNH NĂNG BỔ TRỢ (ANTI-AFK, SPEED, THẢM BAY, NHÌN ĐÊM)
--------------------------------------------------
-- Anti-AFK
task.spawn(function()
...
-- Fullbright (Nhìn trong bóng tối)
task.spawn(function()
...
--------------------------------------------------
-- 2. ESP CỬA THẬT & CỬA GIẢ (DUPE)
--------------------------------------------------
local function applyDoorESP(door)
    if not door:IsA("Model") then return end

    -- Nhận diện cửa giả (Dupe)
    local isFake = door.Name == "DoorFake" or door.Name == "TrickDoor" or door:FindFirstChild("Hidden") or door:FindFirstChild("Dupe")
    
    if door.Name == "Door" or isFake then
        local doorPart = door:FindFirstChild("Door") or door:FindFirstChildWhichIsA("BasePart")
        if not doorPart then return end

        local highlightName = isFake and "Mote_FakeDoorESP" or "Mote_RealDoorESP"
        local highlight = door:FindFirstChild(highlightName) or Instance.new("Highlight")
        ...
        highlight.Enabled = Flags.ESPDoor
        ...
        if isFake then
            ...
            if sign and sign:FindFirstChild("TextLabel") then
                doorNum = " [" .. sign.TextLabel.Text .. "]"
            end
            label.Text = "🚪 CỬA THẬT" .. doorNum
        end
...
--------------------------------------------------
-- 3. ESP VẬT PHẨM (TRONG TỦ, CÔNG TẮC, SÁCH 50 & CẦU CHÌ 100)
--------------------------------------------------
local ValidItems = {
    ["KeyObtain"] = "Chìa khóa",
    ["Key"] = "Chìa khóa",
    ...
    -- Công tắc cửa
    ["LeverForGate"] = "⚡ Công Tắc Cửa",
    ["Lever"] = "⚡ Công Tắc Cửa",
    ["GateButton"] = "⚡ Nút Bấm Cửa",
    
    -- Nhiệm vụ Màn 50 & 100
    ["LiveHintBook"] = "📘 Sách (Cửa 50)",
    ["Book"] = "📘 Sách (Cửa 50)",
    ["HintBook"] = "📘 Sách (Cửa 50)",
    ["FuseInPlainSight"] = "🔋 Cầu Chì (Cửa 100)",
    ["Fuse"] = "🔋 Cầu Chì (Cửa 100)"
}
    ...
    if Flags.MonsterNotify and not notifiedMonsters[obj] then
        ...
        pcall(function()
            StarterGui:SetCore("SendNotification", {
                ...
                Text = displayName .. " vừa xuất hiện!",
                Duration = 4
            })
        end)
    end
...
titleLabel.Text = "MOTE HUB V6.0 - DOORS"
...
-- Tải nút vào Tab ESP
createToggleButton(espTabPage, "ESP Cửa & Dupe", "ESPDoor", 5)
...
pcall(function()
    StarterGui:SetCore("SendNotification", {
        Title = "MOTE HUB V6.0",
        Text = "Đã kích hoạt thành công toàn bộ tính năng!",
        Duration = 4
    })
end)
