-- ==================================================
-- MOTEHUB V6.2 - FIX FULLBRIGHT & THÊM TAB THỬ NGHIỆM
-- ==================================================
...
local Flags = {
    ...
    MonsterNotify = true,
    SpeedHack = false,
    InfiniteJump = false -- Feature mới trong Tab Thử Nghiệm
}

local SpeedMultiplier = 1.5
local CurrentDoorNumber = 0

-- Lưu lại cấu hình Lighting gốc để khôi phục khi tắt Fullbright
...
--------------------------------------------------
-- 1. BỔ TRỢ & SỬA LỖI FULLBRIGHT / INFINITE JUMP
--------------------------------------------------
...
-- Sửa Lỗi Nhìn Trong Bóng Tối (Fullbright Turn Off Fix)
...
task.spawn(function()
    while task.wait(0.2) do
        ...
        elseif isFullbrightActive then
            -- Tắt và khôi phục về môi trường gốc của Doors
...
--------------------------------------------------
-- 2. ESP CỬA (ĐẾM SỐ & PHÂN BIỆT DUPE)
--------------------------------------------------
local function applyDoorESP(door)
    if not door:IsA("Model") or (door.Name ~= "Door" and door.Name ~= "DoorFake") then return end

    local doorPart = door:FindFirstChild("Door") or door:FindFirstChildWhichIsA("BasePart")
    if not doorPart then return end

    local sign = door:FindFirstChild("Sign") or door:FindFirstChild("SignModel")
    local signTextLabel = sign and sign:FindFirstChild("TextLabel", true)

    if not signTextLabel or signTextLabel.Text == "" then return end
    local doorNum = tonumber(signTextLabel.Text)
    ...
    if door.Name == "DoorFake" or door:FindFirstChild("Hidden") or door:FindFirstChild("Dupe") then
        isDupe = true
            ...
            for _, room in ipairs(Workspace:GetChildren()) do
                if room:IsA("Model") and tonumber(room.Name) then
                    ...
                    if roomNum > CurrentDoorNumber then
                        ...
                        if roomPrimary and (roomPrimary.Position - pPos).Magnitude < 80 then
...
--------------------------------------------------
-- 3. ESP VẬT PHẨM (SOI TỦ / HỘC BÀN / RƯƠNG)
--------------------------------------------------
local ValidItems = {
...
task.spawn(function()
    local function processObject(obj)
        pcall(function() applyItemESP(obj); applyMonsterESP(obj); applyDoorESP(obj) end)
    end
...
--------------------------------------------------
-- 6. GIAO DIỆN GUI V6.2 (BỔ SUNG TAB THỬ NGHIỆM)
--------------------------------------------------
...
-- Cụm Nút Tab Navigation (Chia 3 Tab)
...
pcall(function()
    StarterGui:SetCore("SendNotification", {
        Title = "MOTE HUB V6.2",
        Text = "Đã fix lỗi Nhìn Bóng Tối & Thêm Tab Thử Nghiệm!",
        Duration = 4
    })
end)
